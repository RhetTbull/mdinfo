...
**TODO** Update README. The following is really from [autofile](https://github.com/RhetTbull/autofile) which I used to bootstrip mdinfo using the autofile template system.
...
mdinfo uses a template system to specify the target directory and/or filename based on the file's metadata.  For example: 

`mdinfo *.mp3 --target ~/Music --directory "{audio:artist}/{audio:album}"` 
...
mdinfo understands metadata about audio files (MP3, OGG, etc), photo & video files, macOS specific metadata such as Finder tags as well as various filesystem related metadata. 
...
$ mdinfo --target ~/Pictures/FiledPhotos \
...
mdinfo contains a rich templating system which allows fine-grained control   
...
If no default value provided and the field is null, mdinfo will use a default
